...
exports.slaEvaluator = exports.SLAEvaluatorService = void 0;
...
class SLAEvaluatorService {
    ...
    async evaluateRule(rule, context) {
        try {
            ...
            const metrics = JSON.parse(rule.metrics);
     ...
     */
    async calculateMetric(metrics, context) {
     ...
     */
    async calculateResponseTime(context) {
     ...
     */
    async calculateResolutionTime(context) {
        const entity = await this.getEntity(context.entityId, context.entityType);
        if (!entity) {
            return 0;
        }
     ...
     */
    async calculateCustomMetric(metrics, context) {
    ...
    async getEntity(entityId, entityType) {
        switch (entityType) {
            case 'CASE':
                return await index_1.db.case.findUnique({ where: { id: entityId } });
            case 'TASK':
                return await index_1.db.task.findUnique({ where: { id: entityId } });
            case 'HEARING':
                return await index_1.db.hearing.findUnique({ where: { id: entityId } });
            case 'ORDER':
                return await index_1.db.order.findUnique({ where: { id: entityId } });
            case 'PROJECT':
                return await index_1.db.project.findUnique({ where: { id: entityId } });
            default:
     ...
     */
    determineStatus(currentValue, threshold) {
     ...
     */
    async saveEvaluation(result, employeeId) {
        try {
            await index_1.db.sLAEvaluation.create({
                data: {
    ...
    async getEntityEvaluations(entityId, entityType) {
        return await index_1.db.sLAEvaluation.findMany({
            where: {
                entityId,
                entityType
            },
     ...
     */
    async getSLABreachSummary(teamId, dateRange) {
        const whereConditions = {
            status: {
    ...
    async createDefaultSLARules() {
        ...
        for (const rule of defaultRules) {
            try {
                await index_1.db.sLARule.create({
                    data: rule
                });
            }
            catch (error) {
...
exports.SLAEvaluatorService = SLAEvaluatorService;
// Export singleton instance
exports.slaEvaluator = new SLAEvaluatorService();
