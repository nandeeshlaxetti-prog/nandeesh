...
exports.automationTriggersService = exports.AutomationTriggersService = void 0;
 ...
 */
class AutomationTriggersService {
    /**
    ...
    async triggerHearingScheduled(hearingId) {
        try {
            ...
            await automation_service_1.automationService.handleHearingScheduled(hearingId);
        }
        catch (error) {
    ...
    async triggerOrderPdfAdded(orderId) {
        try {
            ...
            await automation_service_1.automationService.handleOrderPdfAdded(orderId);
        }
        catch (error) {
    ...
    async triggerTaskBlocked(taskId) {
        try {
            ...
            await automation_service_1.automationService.handleTaskBlocked(taskId);
        }
        catch (error) {
     ...
     */
    async triggerCaseStatusChanged(caseId, oldStatus, newStatus) {
        try {
            console.log(`🎯 Triggering case status changed automation: ${caseId} (${oldStatus} → ${newStatus})`);
            // Get case data
            const caseData = await index_1.db.case.findUnique({
                where: { id: caseId },
                include: {
                    assignedLawyer: true,
                    team: true
     ...
     */
    async triggerTaskStatusChanged(taskId, oldStatus, newStatus) {
        try {
            console.log(`🎯 Triggering task status changed automation: ${taskId} (${oldStatus} → ${newStatus})`);
            // Get task data
            const taskData = await index_1.db.task.findUnique({
                where: { id: taskId },
                include: {
                    assignee: true,
                    creator: true,
     ...
     */
    async triggerDocumentUploaded(documentId) {
        try {
            console.log(`🎯 Triggering document uploaded automation: ${documentId}`);
            // Get document data
            const document = await index_1.db.document.findUnique({
                where: { id: documentId },
                include: {
                    case: true,
                    uploader: true
            ...
            if (document.mimeType === 'application/pdf' && document.filename.toLowerCase().includes('order')) {
                ...
                const order = await index_1.db.order.findFirst({
                    where: {
     ...
     */
    async createCaseClosedTasks(caseData) {
        try {
            console.log(`📝 Creating case closed tasks for case: ${caseData.id}`);
            // Create case closure task
            await automation_service_1.automationService.sendNotification({
     ...
     * Notify case on hold
     */
    async notifyCaseOnHold(caseData) {
        try {
            ...
            if (caseData.teamId) {
                await automation_service_1.automationService.sendNotificationToTeam(caseData.teamId, {
                    title: 'Case On Hold',
                    message: `Case ${caseData.caseNumber} has been put on hold.`,
                    priority: 'MEDIUM',
                    entityType: 'CASE',
    ...
    async notifyTaskCompleted(taskData) {
        try {
            ...
            if (taskData.case?.teamId) {
                const team = await index_1.db.team.findUnique({
     ...
     */
    async triggerHearingPrepTask(hearingId) {
        try {
            ...
            const task = await index_1.db.task.create({
                data: {
     ...
     */
    async triggerOrderProcessingTask(orderId) {
        try {
     ...
     */
    async triggerBlockedTaskNotification(taskId) {
        try {
     ...
     */
    async getAutomationTriggersStatus() {
        try {
            return {
                isActive: true,
                triggers: {
...
exports.AutomationTriggersService = AutomationTriggersService;
// Export singleton instance
exports.automationTriggersService = new AutomationTriggersService();
